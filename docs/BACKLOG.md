...
- 状态：已完成。新分析保存 evidence/Prompt 双指纹，可从详情克隆冻结输入；对比页只把双指纹一致的记录认定为严格 A/B。默认核心包固定最多 8 条并优先不同主题，相关度权重不变。
- 状态：已增加本地证据质量 A-D 等级和主假设、反证、翻转条件、行动含义；Provider 原始置信度不再作为校准后概率展示，也不跨 Provider 平均。
- 状态：已增加 `/analyze/stability` 和只读 CLI，执行 Top-4/6/8、leave-one-out 与同窗口可比性审计。最近 50 条完成分析中 10 个重复窗口均不是严格同输入，说明旧样本适合发现问题、不适合精确 Provider 归因。
- 验收：纯函数覆盖同输入、顺序变化、方向冲突、内部冲突、重复/越界引用、Provider 置信度独立性和不修改原始 run；真实库 50 条离线审计约 0.06 秒。
