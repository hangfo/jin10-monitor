...
- 归档 2026-07-11 至 2026-07-17 新增的 15 个自然分析样本只读审计：确认方向冲突与强方向一致样本均已出现，记录 JSON/引用完整性、Provider 分歧、置信度偏差和证据集敏感性，后续先设计确定性证据包与敏感度/校准层，不立即修改 judgement Prompt 或 evidence scoring。

## 2026-07-17

- 修复近期快讯详情页的默认 ± 行情窗口：未来半边自动截到当前可用分钟，极近或带轻微源时间偏移的快讯也会回退为有效的尾随窗口，确保页面首次打开即可加载已有 K 线；手动输入未来时间仍继续拦截，“生成证据包”跳转复用同一有效窗口。
- 修复快讯红线依赖行情源恰好返回对应 K 线的问题：红线现在按所选窗口和 1m/5m 周期建立不含价格的时间锚点，最新 K 线尚未形成或中间缺根时仍能显示，且不伪造 OHLC/成交量；拖出和移回可视范围继续正确隐藏与恢复。
- 修复共享日期时间校验脚本晚于页面业务脚本初始化导致的 Dashboard 回退：分析页重新支持 BTC/ETH 自动匹配 BTCUSDT/ETHUSDT，快讯详情页首次打开及修改有效时间后均能正常加载行情图；未来时间和反向时间继续在外部行情请求前拦截。
- 加固行情图快讯红线的可视范围生命周期：拖动、缩放和窗口重载时按动画帧重算位置，移出可视范围后移回可可靠恢复；请求窗口不包含快讯时不再把红线错误回退到图表中央，重新包含后恢复到对应周期 K 线。
- 修复长时间关机后的自动补抓截断：启动恢复现在从上次有效入库游标完整覆盖到本次启动时间，按小时分片并使用独立断点续传；单个子窗口达到入库上限会标记为未完成并保留断点，不再因 24 小时或全局 1000 条上限静默遗漏历史消息。
- 收紧历史回溯与 Telegram 边界：任意 `--from/--to` 手动补抓默认按 60 分钟分片、幂等入库且不发送 Telegram；只有显式 `--catch-up-telegram` 才按既有筛选规则发送，自动补抓继续只允许一条摘要，不逐条补发历史消息。
- 为 Dashboard 全部四个日期时间输入增加统一前后端校验：开始/结束必填且有效、均不得晚于当前时间、开始必须早于结束；分析窗口另限制为最长 7 天，行情 API 在调用 adapter 前拒绝非法窗口，避免无效外部请求和过大本地证据扫描。
- 生产恢复 2026-07-16 07:37:52 至 2026-07-17 03:05:02 的实际离线缺口：按 30 分钟窗口从金十原始 REST 幂等补入 1278 条，指定的 15:00-17:00 窗口精确恢复 193 条；未补发历史 Telegram，实时游标继续单调推进。
