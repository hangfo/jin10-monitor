...
def dashboard_compact_text(*parts: object, limit: int = 120) -> str:
    text = compact_text(" ".join(str(part or "") for part in parts if part), limit=limit)
    return text or "（无标题）"


def dashboard_priority_options() -> list[tuple[str, str]]:
    return [
        ("", "全部"),
        (PRIORITY_IMPORTANT, PRIORITY_LABELS.get(PRIORITY_IMPORTANT, PRIORITY_IMPORTANT)),
        (PRIORITY_HIGH, PRIORITY_LABELS.get(PRIORITY_HIGH, PRIORITY_HIGH)),
        (PRIORITY_NORMAL, PRIORITY_LABELS.get(PRIORITY_NORMAL, PRIORITY_NORMAL)),
        (PRIORITY_NONE, PRIORITY_LABELS.get(PRIORITY_NONE, PRIORITY_NONE)),
    ]


def query_dashboard_recent_items(*, limit: int = 80, priority: str = "", with_status: bool = False) -> list[sqlite3.Row]:
    clauses = []
    params: list[object] = []
    if priority:
...
def query_dashboard_telegram_status(status_filter: str = "problem", *, limit: int = 80) -> list[sqlite3.Row]:
    params: list[object] = []
    where = ""
    if status_filter == "problem":
        where = "WHERE t.status IN (?, ?, ?)"
    ...
    with open_readonly_history_db() as conn:
        return conn.execute(
            f"""
            SELECT t.updated_at, t.status, t.mode, t.message_id, t.detail,
                   h.published_at, h.priority_level, h.title, h.content, h.source
            FROM telegram_delivery_status t
            LEFT JOIN flash_history h ON h.id = t.message_id
            {where}
            ORDER BY t.updated_at DESC
...
def query_dashboard_message_status(message_id: str) -> Optional[sqlite3.Row]:
    with open_readonly_history_db() as conn:
        return conn.execute(
            """
            SELECT message_id, channel, mode, status, detail, updated_at
...
th {{ font-size: 12px; color: var(--muted); font-weight: 700; background: #f1f4f8; }}
...
.pill.sent {{ color: var(--ok); background: #e8f5ee; }}
.pill.failed {{ color: var(--danger); background: #fdeeee; }}
.pill.unknown_timeout {{ color: var(--warn); background: #fff3d6; }}
.pill.skipped {{ color: #475467; background: #eceff3; }}
.grid {{ display: grid; grid-template-columns: minmax(0, 1fr) 320px; gap: 18px; }}
.panel {{ background: var(--panel); border: 1px solid var(--line); border-radius: 8px; padding: 16px; }}
.panel h2 {{ margin: 0 0 12px; font-size: 18px; }}
.kv {{ display: grid; grid-template-columns: 108px 1fr; gap: 8px 12px; }}
.timeline {{ display: grid; gap: 10px; }}
.event {{ border: 1px solid var(--line); border-radius: 8px; background: var(--panel); padding: 12px; }}
.event.center {{ border-color: var(--accent); box-shadow: inset 3px 0 0 var(--accent); }}
...
.card-text {{ line-height: 1.55; overflow-wrap: anywhere; }}
.card-meta {{ display: flex; gap: 8px; flex-wrap: wrap; margin-top: 10px; color: var(--muted); font-size: 13px; }}
.card-action {{ margin-left: auto; color: var(--accent); font-weight: 700; }}
.refresh-note {{ color: var(--muted); font-size: 13px; margin-top: 6px; }}
.empty, .error {{ background: var(--panel); border: 1px solid var(--line); border-radius: 8px; padding: 18px; }}
...
@media (max-width: 820px) {{
  ...
  .button input[type="checkbox"] {{ width: 18px; height: 18px; }}
...
<header class="topbar">
  <div class="brand">
    <h1>Jin10 Monitor Dashboard</h1>
    <p>本地只读控制台 · SQLite mode=ro · 不发送 Telegram</p>
  </div>
  <nav class="nav">
    <a href="/">最近快讯</a>
    <a href="/telegram-status">Telegram 状态</a>
    <a href="/aggregation-report">聚合报告</a>
  </nav>
</header>
{body}
</div>
...
def dashboard_status_pill(status: object) -> str:
    text = str(status or "")
    if not text:
        return '<span class="muted">无</span>'
    safe = escape(text)
    return f'<span class="pill {safe}">{safe}</span>'


def dashboard_priority_pill(priority: object) -> str:
    text = str(priority or PRIORITY_NONE)
    label = PRIORITY_LABELS.get(text, text)
    return f'<span class="pill">{escape(label)}</span>'


def dashboard_error_page(message: str) -> bytes:
    return html_page("Dashboard 错误", f'<main class="error"><strong>无法读取历史库</strong><p>{escape(message)}</p></main>')


def parse_dashboard_int(query: dict[str, list[str]], name: str, default: int, minimum: int, maximum: int) -> int:
...
def render_dashboard_index(query: dict[str, list[str]]) -> bytes:
    limit = parse_dashboard_int(query, "limit", 80, 1, 300)
    priority = (query.get("priority") or [""])[0]
    allowed_priorities = {value for value, _label in dashboard_priority_options()}
    if priority not in allowed_priorities:
        priority = ""
    with_status = (query.get("with_status") or [""])[0] == "1"
    rows = query_dashboard_recent_items(limit=limit, priority=priority, with_status=with_status)
    options = "\n".join(
        f'<option value="{escape(value)}"{" selected" if value == priority else ""}>{escape(label)}</option>'
        for value, label in dashboard_priority_options()
    )
    checked = " checked" if with_status else ""
    body_rows = []
    mobile_cards = []
    for row in rows:
        item_url = f"/item/{urllib.parse.quote(str(row['id']))}"
        text = dashboard_compact_text(row["title"], row["content"], limit=96)
        source = escape(str(row["source"] or ""))
        body_rows.append(
            "<tr>"
            f"<td class=\"mono\"><a href=\"{item_url}\">{escape(str(row['id']))}</a></td>"
            f"<td>{escape(str(row['published_at'] or ''))}<div class=\"muted\">入库 {escape(str(row['created_at'] or ''))}</div></td>"
            f"<td>{dashboard_priority_pill(row['priority_level'])}</td>"
            f"<td>{escape(text)}<div class=\"muted\">{escape(str(row['news_source'] or ''))}</div></td>"
            f"<td>{source}</td>"
            f"<td>{dashboard_status_pill(row['telegram_status'])}<div class=\"muted\">{escape(str(row['telegram_mode'] or ''))}</div></td>"
            "</tr>"
        )
        mobile_cards.append(
            f'<a class="item-card" href="{item_url}">'
            '<div class="card-head">'
            f'<div>{dashboard_priority_pill(row["priority_level"])}</div>'
            f'<div class="mono">{escape(str(row["id"]))}</div>'
            '</div>'
            f'<div class="card-text">{escape(text)}</div>'
            '<div class="card-meta">'
            f'<span>{escape(str(row["published_at"] or ""))}</span>'
            f'<span>{source}</span>'
            f'<span>{dashboard_status_pill(row["telegram_status"])}</span>'
            '<span class="card-action">查看上下文</span>'
            '</div>'
            '</a>'
        )
    table = (
        '<div class="empty">暂无匹配快讯。</div>'
        if not body_rows
        else "<div class=\"table-wrap\"><table><thead><tr>"
        "<th>消息 ID</th><th>时间</th><th>优先级</th><th>内容</th><th>来源</th><th>Telegram</th>"
        "</tr></thead><tbody>"
        + "\n".join(body_rows)
        + "</tbody></table></div>"
...
<main>
  <section class="toolbar">
    <div>
      <h2>最近快讯</h2>
      <div class="muted">默认按发生时间倒序，已加载 {len(rows)} 条；点击任意消息查看前后上下文。</div>
      <div class="refresh-note">页面每 15 秒自动刷新一次，也可以用浏览器刷新立即更新。</div>
    </div>
    <form class="filters" method="get" action="/">
      <select name="priority" aria-label="优先级">{options}</select>
      <label class="button"><input type="checkbox" name="with_status" value="1"{checked}> 只看 Telegram 状态</label>
      <input name="limit" value="{limit}" inputmode="numeric" aria-label="条数">
...
"""
    return html_page("最近快讯", body)


def render_dashboard_item(message_id: str, query: dict[str, list[str]]) -> bytes:
    minutes = parse_dashboard_int(query, "minutes", 15, 0, 120)
    center, rows = query_context(message_id, minutes=minutes)
    if center is None:
        return html_page("未找到消息", f'<main class="empty">未找到消息：<span class="mono">{escape(message_id)}</span></main>')
    center_status = query_dashboard_message_status(message_id)
    timeline = []
    for row in rows:
        is_center = str(row["id"]) == message_id
        text = dashboard_compact_text(row["title"], row["content"], limit=220)
        timeline.append(
            f'<article class="event{" center" if is_center else ""}">'
            '<div class="event-head">'
            f'<div><strong>{escape(str(row["published_at"] or ""))}</strong> {dashboard_priority_pill(row["priority_level"])}</div>'
            f'<a class="mono" href="/item/{urllib.parse.quote(str(row["id"]))}">{escape(str(row["id"]))}</a>'
            '</div>'
            f'<div>{escape(text)}</div>'
            f'<div class="muted">{escape(str(row["news_source"] or ""))}</div>'
            '</article>'
        )
    side = f"""
<aside class="panel">
  <h2>消息详情</h2>
  <div class="kv">
    <div class="muted">消息 ID</div><div class="mono">{escape(message_id)}</div>
    <div class="muted">发生时间</div><div>{escape(str(center["published_at"] or ""))}</div>
    <div class="muted">优先级</div><div>{dashboard_priority_pill(center["priority_level"])}</div>
    <div class="muted">Telegram</div><div>{dashboard_status_pill(center_status["status"] if center_status else "")}</div>
    <div class="muted">来源</div><div>{escape(str(center["news_source"] or ""))}</div>
    <div class="muted">来源链接</div><div>{escape(str(center["source_url"] or ""))}</div>
    <div class="muted">图片</div><div>{escape(str(center["pic_url"] or ""))}</div>
  </div>
</aside>
"""
    body = f"""
<main>
  <section class="toolbar">
    <div>
      <h2>单条详情 + 上下文</h2>
      <div class="muted">前后 {minutes} 分钟，共 {len(rows)} 条；中心消息以左侧色条标记。</div>
    </div>
    <form class="filters" method="get" action="/item/{escape(urllib.parse.quote(message_id))}">
      <input name="minutes" value="{minutes}" inputmode="numeric" aria-label="上下文分钟数">
      <button type="submit">更新窗口</button>
    </form>
  </section>
  <section class="grid">
    <div class="timeline">{''.join(timeline) if timeline else '<div class="empty">消息缺少可解析时间，无法查询上下文。</div>'}</div>
    {side}
  </section>
</main>
"""
    return html_page("消息详情", body)


def render_dashboard_telegram_status(query: dict[str, list[str]]) -> bytes:
    status_filter = (query.get("status") or ["problem"])[0]
    if status_filter not in {"problem", TELEGRAM_STATUS_FAILED, TELEGRAM_STATUS_UNKNOWN_TIMEOUT, TELEGRAM_STATUS_SKIPPED, TELEGRAM_STATUS_SENT, "all"}:
        status_filter = "problem"
    limit = parse_dashboard_int(query, "limit", 80, 1, 300)
    rows = query_dashboard_telegram_status(status_filter, limit=limit)
    status_options = [
        ("problem", "问题状态"),
        (TELEGRAM_STATUS_FAILED, "failed"),
        (TELEGRAM_STATUS_UNKNOWN_TIMEOUT, "unknown_timeout"),
    ...
    mobile_cards = []
    for row in rows:
        item_url = f"/item/{urllib.parse.quote(str(row['message_id']))}"
        text = dashboard_compact_text(row["title"], row["content"], limit=90)
        body_rows.append(
            "<tr>"
            f"<td>{escape(str(row['updated_at'] or ''))}</td>"
            f"<td>{dashboard_status_pill(row['status'])}<div class=\"muted\">{escape(str(row['mode'] or ''))}</div></td>"
            f"<td class=\"mono\"><a href=\"{item_url}\">{escape(str(row['message_id']))}</a></td>"
            f"<td>{escape(text)}<div class=\"muted\">{escape(str(row['published_at'] or ''))}</div></td>"
            f"<td>{escape(compact_text(str(row['detail'] or ''), limit=120))}</td>"
            "</tr>"
        )
        mobile_cards.append(
            f'<a class="item-card" href="{item_url}">'
            '<div class="card-head">'
            f'<div>{dashboard_status_pill(row["status"])}<div class="muted">{escape(str(row["mode"] or ""))}</div></div>'
            f'<div class="mono">{escape(str(row["message_id"]))}</div>'
            '</div>'
            f'<div class="card-text">{escape(text)}</div>'
            '<div class="card-meta">'
            f'<span>{escape(str(row["updated_at"] or ""))}</span>'
            f'<span>{escape(compact_text(str(row["detail"] or ""), limit=80))}</span>'
            '<span class="card-action">查看上下文</span>'
            '</div>'
            '</a>'
        )
    table = (
        '<div class="empty">暂无 Telegram 投递状态记录。</div>'
        if not body_rows
        else "<div class=\"table-wrap\"><table><thead><tr>"
        "<th>更新时间</th><th>状态</th><th>消息 ID</th><th>内容</th><th>详情</th>"
        "</tr></thead><tbody>"
        + "\n".join(body_rows)
        + "</tbody></table></div>"
        + '<div class="mobile-list">'
...
<main>
  <section class="toolbar">
    <div>
      <h2>Telegram 投递状态</h2>
      <div class="muted">只读诊断视图，已加载 {len(rows)} 条；点击任意记录查看消息上下文。</div>
    </div>
    <form class="filters" method="get" action="/telegram-status">
      <select name="status" aria-label="状态">{options}</select>
      <input name="limit" value="{limit}" inputmode="numeric" aria-label="条数">
      <button type="submit">筛选</button>
    </form>
  </section>
...
"""
    return html_page("Telegram 状态", body)


def render_dashboard_aggregation_report() -> bytes:
    body = """
<main>
  <section class="toolbar">
    <div>
      <h2>聚合候选报告</h2>
      <div class="muted">此页为只读占位，等待后续 --aggregation-report CLI 落地后接入。</div>
    </div>
  </section>
  <section class="panel">
    <h2>当前边界</h2>
    <p>Dashboard 不开启 AGGREGATION_V2，不修改聚合配置，也不执行 suppress。</p>
    <p>后续这里应展示候选组数量、可能被压掉的消息和疑似误压样本。</p>
  </section>
</main>
"""
    return html_page("聚合候选报告", body)


class DashboardHandler(http.server.BaseHTTPRequestHandler):
    def log_message(self, fmt: str, *args: object) -> None:
        log.info("dashboard %s - %s", self.address_string(), fmt % args)
    ...
    def do_GET(self) -> None:
        ...
        try:
            if parsed.path == "/":
                content = render_dashboard_index(query)
            elif parsed.path.startswith("/item/"):
                message_id = urllib.parse.unquote(parsed.path.removeprefix("/item/"))
                content = render_dashboard_item(message_id, query)
            elif parsed.path == "/telegram-status":
                content = render_dashboard_telegram_status(query)
            elif parsed.path == "/aggregation-report":
                content = render_dashboard_aggregation_report()
            else:
                self.send_html(html_page("404", '<main class="empty">页面不存在。</main>'), status=404)
                return
        except FileNotFoundError as exc:
            content = dashboard_error_page(f"历史库不存在：{exc}")
        except sqlite3.OperationalError as exc:
            content = dashboard_error_page(f"历史库读取失败：{exc}")
        self.send_html(content)

    def do_HEAD(self) -> None:
        parsed = urllib.parse.urlparse(self.path)
        if parsed.path == "/" or parsed.path == "/telegram-status" or parsed.path == "/aggregation-report" or parsed.path.startswith("/item/"):
            self.send_head_ok()
        else:
...
def run_dashboard(host: str = "127.0.0.1", port: int = 8765) -> None:
    if host not in {"127.0.0.1", "localhost"}:
        raise SystemExit("Dashboard MVP 仅允许绑定 127.0.0.1 / localhost")
    server = http.server.ThreadingHTTPServer((host, port), DashboardHandler)
    log.info("Dashboard 已启动：http://%s:%s", host, port)
    log.info("Dashboard 只读打开 HISTORY_DB=%s，不发送 Telegram、不触发 REST", HISTORY_DB)
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dashboard", action="store_true", help="启动本地只读 Dashboard")
    parser.add_argument("--dashboard-host", default="127.0.0.1", help="Dashboard 监听地址，MVP 仅允许 127.0.0.1 / localhost")
    parser.add_argument("--dashboard-port", type=int, default=8765, help="Dashboard 监听端口，默认 8765")
...
if __name__ == "__main__":
    try:
        ...
        elif args.telegram_status is not None:
